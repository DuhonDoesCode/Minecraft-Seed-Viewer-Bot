...
// This is just like the biome_struct.c function, except WAY more lenient (for finding rare things usually, like a mush biome spawn next to a mansion. Btw, that's seed 288406.)
// It is WAY slower and does NOT cap itself at 100000 seeds, so be careful if implementing this.
...
int find(int biome, char *biomeName, int version, int structure, int start){
    ...
    for (seed = start + 1; ; seed++)
    {
        ...
        if (biomeID == biome)
        {
            ...
            if (result && !(p.x >= 4096 || p.z >= 4096) && isViableStructurePos(structure, &g, p.x, p.z, 0)){
                FILE *fptr;
                fptr = fopen(".\\tmp.txt","w");
